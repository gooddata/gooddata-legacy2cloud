...
escaped_glob_replacement = re.compile(
    "({})".format("|".join(escaped_glob_tokens_to_re).replace("\\", "\\\\\\"))
)
...
def load_ignore_file(ignore_path: Path, verbose: bool) -> list[re.Pattern[AnyStr]]:
    ...
    result = [
        re.compile(glob_to_re(line.strip()))
...
def add_copyright(file_name: Path, insert_line: int) -> None:
    ...
    if file_size < 1:
        with open(file_name, "w", encoding="utf-8") as f:
            f.write(
                comment_copyright(
                    file_name.suffix, f"(C) {str(today.year)} GoodData Corporation"
                )
            )
            f.write("\n")
    else:
            ...
            for pos, line in enumerate(f):
                if pos == insert_line:
                    print(
                        comment_copyright(
                            file_name.suffix,
                            f"(C) {str(today.year)} GoodData Corporation",
                        )
                    )
                print(line, end="")
...
def process_files(
    file_names: list[str],
    ignore_list: list[re.Pattern[AnyStr]],
    update_file: bool,
    verbose: bool,
) -> int:
    ret_val = 0
        ...
        if not is_ignored_file(ignore_list, object_path):
            if object_path.is_dir():
                ret_val |= process_folder(
                    object_path, ignore_list, update_file, verbose
                )
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Update file(s) with copyright",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        default=False,
        help="Print more detailed information about copyright scan",
    )

    subparsers = parser.add_subparsers(
        dest="tool_mode", help="tool mode", required=True
    )
    files_parser = subparsers.add_parser(
        "FILES", help="Accept files to check for copyright"
    )
    files_parser.add_argument(
        "file_names", nargs="*", help="One or more files to check for copyright"
    )

    all_parser = subparsers.add_parser(
        "FOLDER", help="Scan folder for the files to check for copyright"
    )
    all_parser.add_argument(
        "--folder", action="store", default=".", help="Path to the folder to scan"
    )
...
def main() -> int:
    ...
    if args.tool_mode == "FILES":
        ...
        ret_val = process_folder(
            Path(args.folder), ignore_list, args.update, args.verbose
        )
    else:
