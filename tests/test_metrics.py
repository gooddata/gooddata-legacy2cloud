...
"""
This test aims to verify the transformation of Platform metrics to Cloud metrics.

All calls to Platform and Cloud are mocked, the data is loaded from JSON files stored
in the `tests/data/metrics` directory.

The test verifies that the transformation of Platform metrics metadata matches
...
def test_metrics_migration(
    ...
    """Test the transformation of Platform metrics to Cloud metrics.
    ...
    - <case_file_name>_platform.json - Platform metrics metadata
    - <case_file_name>_cloud.json - Expected Cloud metrics metadata
    """

    # Load Platform metrics
    platform_metrics = load_json(f"{TEST_CASES_DIR}/{case_file_name}_platform.json")
    assert isinstance(platform_metrics, list), "Platform metrics should be a list"
