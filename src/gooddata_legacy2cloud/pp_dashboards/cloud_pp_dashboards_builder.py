...
"""Builder for transforming Legacy pixel perfect dashboards to Cloud dashboards."""
...
import gooddata_legacy2cloud.pp_dashboards.cloud_objects.dashboard_objects as pdo
import gooddata_legacy2cloud.pp_dashboards.cloud_objects.filter_objects as ppf
import gooddata_legacy2cloud.pp_dashboards.cloud_objects.visualisation_objects as pvo
import gooddata_legacy2cloud.pp_dashboards.utils as pp_utils
from gooddata_legacy2cloud.helpers import PP_FILTER_CONTEXT_PREFIX, PP_INSIGHT_PREFIX
from gooddata_legacy2cloud.pp_dashboards.data_classes import PPDashboardContext
from gooddata_legacy2cloud.pp_dashboards.grid_maker import (
    GridConfig,
    migrate_to_sections,
)
from gooddata_legacy2cloud.pp_dashboards.helpers import (
    extract_values_by_key,
    prefetch_legacy_objects,
)
...
class CloudPixelPerfectDashboardsBuilder:
    """Builds Cloud dashboards from Legacy pixel perfect dashboards.

    This builder processes Legacy pixel perfect dashboards and transforms them into
    Cloud responsive dashboards. By default, one Legacy dashboard becomes one
    ...
    def process_legacy_dashboards(
        self,
        legacy_dashboards: list[dict],
        skip_deploy: bool = False,
        overwrite_existing: bool = False,
    ) -> None:
        """Process all Legacy pixel perfect dashboards.

        Args:
            legacy_dashboards: List of Legacy dashboard definitions
        ...
        for dashboard_idx, legacy_dashboard in enumerate(legacy_dashboards):
            if not legacy_dashboard["projectDashboard"]["meta"]["title"]:
                continue

            dashboard_title = legacy_dashboard["projectDashboard"]["meta"]["title"]
            logger.info("Processing %d: %s", dashboard_idx + 1, dashboard_title)

            self.process_single_dashboard(legacy_dashboard, overwrite_existing)

    def process_single_dashboard(
        self, legacy_dashboard_raw: dict, overwrite_existing: bool
    ) -> None:
        """Process a single Legacy dashboard and all its tabs.

        Args:
            legacy_dashboard_raw: Raw Legacy dashboard definition
            overwrite_existing: If True, overwrite existing dashboards
        """
        # Extract and prefetch all 'obj' values from the raw Legacy dashboard definition
        obj_values = extract_values_by_key(legacy_dashboard_raw, "obj")

        # Prefetch Legacy objects concurrently to warm up any underlying caches
        prefetch_legacy_objects(self.ctx, list(set(obj_values)))

        # Pre fetch Legacy filter elements to extend cache
        obj_values = extract_values_by_key(self.ctx.legacy_client.cache, "elements")
        prefetch_legacy_objects(self.ctx, list(set(obj_values)))

        # Validate dashboard using pydantic model
        legacy_dashboard = PixelPerfectDashboard.model_validate(legacy_dashboard_raw)
        # Set context and initialize nested objects
        legacy_dashboard.ctx = self.ctx
        legacy_dashboard.initialize_nested_objects()
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = [
                    executor.submit(
                        self.process_tab,
                        legacy_dashboard,
                        legacy_dashboard_raw,
                        tab_idx,
                        tab,
                        overwrite_existing,
                    )
                    for tab_idx, tab in enumerate(legacy_dashboard.content.tabs)
                ]
        ...
        self._process_dashboard_as_tabbed(
            legacy_dashboard, legacy_dashboard_raw, overwrite_existing
        )

    def _process_dashboard_as_tabbed(
        self,
        legacy_dashboard: PixelPerfectDashboard,
        legacy_dashboard_raw: dict,
        ...
        if self.ctx.mapping_logger:
            self.ctx.mapping_logger.write_identifier_relation(
                legacy_dashboard.meta.identifier,
                cloud_dashboard.id,
            )

        # Set dashboard public flag (same logic as regular migration)
        cloud_dashboard._public = not legacy_dashboard.meta.unlisted or (
            legacy_dashboard.meta.unlisted == 0
        )

        if self._should_skip_or_overwrite(
            cloud_dashboard, legacy_dashboard.meta.title, overwrite_existing
        ):
            return

        migrated_tabs = 0
        for _tab_idx, tab in enumerate(legacy_dashboard.content.tabs):
        ...
        self._log_transformation_tabbed(
            legacy_dashboard, legacy_dashboard_raw, cloud_dashboard
        )
    ...
    def process_tab(
        ...
        Args:
            legacy_dashboard: Validated Legacy dashboard object
            legacy_dashboard_raw: Raw Legacy dashboard definition
        ...
        cloud_min_dashboard = self._create_min_dashboard(legacy_dashboard, tab, tab_idx)
        if self._should_skip_or_overwrite(
    ...
    def _create_min_dashboard(
        self, legacy_dashboard: PixelPerfectDashboard, tab: Any, tab_idx: int
    ) -> pdo.CloudDashboard:
        """Create a minimal Cloud dashboard object from Legacy definition.

        Sets the public flag based on Legacy metadata (unlisted == 0 or missing -> public).

        Args:
            legacy_dashboard: Legacy dashboard object
            tab: Tab to convert
            tab_idx: Index of the tab
        ...
        """
        cloud_min_dashboard = pdo.CloudDashboard.create_from_legacy_definition(
            pixel_perfect_dashboard=legacy_dashboard,
            tab=tab,
            tab_idx=tab_idx,
        )
        # Write mapping between Legacy and Cloud dashboards
        if self.ctx.mapping_logger:
            self.ctx.mapping_logger.write_identifier_relation(
                f"{legacy_dashboard.meta.identifier}_{tab.identifier}",
                cloud_min_dashboard.id,
            )
        # Set dashboard public flag (same logic as regular migration)
        cloud_min_dashboard._public = not legacy_dashboard.meta.unlisted or (
            legacy_dashboard.meta.unlisted == 0
        )
        return cloud_min_dashboard
        ...
        for item_order, item in enumerate(
            ...
            if item.type == "headlineItem":
                ...
                visualization_title = pp_utils.get_migration_id(
                    ...
                    legacy_identifier=item.metric.meta.identifier,
                )
                ...
                visualization_id = visualisation_maker.create_visualization(
                    ...
                    legacy_measure_identifier=item.metric.meta.identifier,
                    title=item.title,
    ...
    def _log_transformation_tabbed(
        ...
        if self.ctx.transformation_logger:
            self.ctx.transformation_logger.write_transformation(
                title=f"{legacy_dashboard.meta.title}",
                legacy_object=json.dumps(
                    {
                        "projectDashboard": {
                            "meta": legacy_dashboard_raw["projectDashboard"]["meta"],
                            "content": legacy_dashboard_raw["projectDashboard"][
                                "content"
                            ],
                        }
                    }
    ...
    def _log_transformation(
        ...
        if self.ctx.transformation_logger:
            self.ctx.transformation_logger.write_transformation(
                title=f"{legacy_dashboard.meta.title} - {tab.title}",
                legacy_object=json.dumps(
                    {
                        "projectDashboard": {
                            "meta": legacy_dashboard_raw["projectDashboard"]["meta"],
                            "content": {
                                "tabs": legacy_dashboard_raw["projectDashboard"][
                                    "content"
                                ]["tabs"][tab_idx],
                                "filters": legacy_dashboard_raw["projectDashboard"][
                                    "content"
                                ]["filters"],
                            },
                        }
