...
from gooddata_legacy2cloud.dashboards.drill_converter import (
    CurrentDashboard,
    DrillConverter,
)
...
class CloudDashboard:
    """
    The CloudDashboard class is responsible for transforming the Legacy dashboard to Cloud format.
    """
    ...
    def _get_rich_text_for_missing_insight(self, insight_uri):
        ...
        workspace_id = self.ctx.legacy_client.pid
        insight_id = insight_uri.rsplit("/")[-1]
        legacy_obj_uri = f"{legacy_domain}/analyze/#/{workspace_id}/{insight_id}/edit"
...
You can view the original insight [here]({legacy_obj_uri})."""
    ...
    def _get_insight_item(self, obj):
        insight_obj = self.ctx.legacy_client.get_object(
    ...
    def _get_dataset_item(self, dataset_uri: str):
        dataset_obj = self.ctx.legacy_client.get_object(dataset_uri)
        return {
            "identifier": {
    ...
    def _get_ignore_dashboard_filters(self, obj: dict, widget_type: str) -> list:
        ...
        for item in obj[widget_type]["content"]["ignoreDashboardFilters"]:
            filter_obj = self.ctx.legacy_client.get_object(
    ...
    def _get_widget_drills(self, obj: dict, widget_type: str) -> list:
        """
        Extract and convert drills from a Legacy widget to Cloud format.
        """
        ...
        current_dashboard = CurrentDashboard(
            legacy_id=self.meta["identifier"], cloud_id=self.cloud_dashboard_id
        )
    ...
    def _resolve_widget_type(self, widget_object: dict) -> Literal["kpi", "insight"]:
        ...
        if "visualizationWidget" in widget_object:
            ...
            if content_visualization_class:
                ...
                raw_visualization_class = self.ctx.legacy_client.get_object(
    ...
    def _get_widget(self, widget_uri: str) -> WidgetWrapper:
        obj = self.ctx.legacy_client.get_object(widget_uri)
