...
def get_cloud_id_date_dimension(legacy_id):
    """
    Returns the date dimension ID.
    """
    return f"dt_{legacy_id}"
...
def get_unique_id(logger, legacy_attr_id, attr_id):
    """
    ...
    # Check if any existing keys are similar to our Legacy identifier
    has_conflicting_key = any(
        are_identifiers_similar(key, legacy_attr_id) for key in keys_for_attr_id
    )
    ...
    return get_unique_identifier_with_suffix(logger, legacy_attr_id, attr_id)


def get_unique_identifier_with_suffix(logger, legacy_attr_id, base_attr_id):
    """
    Helper method to find a unique identifier by appending numeric suffixes.

    Args:
        legacy_attr_id: The original Legacy identifier to check similarity against
    ...
    while True:
        ...
        # Check if any keys using this candidate are similar to our Legacy identifier
        candidate_has_similar_conflict = any(
            are_identifiers_similar(key, legacy_attr_id) for key in keys_using_candidate
        )
