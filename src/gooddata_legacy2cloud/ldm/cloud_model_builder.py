...
the model for Cloud from the Legacy model. It includes functionality for
...
from gooddata_legacy2cloud.helpers import prefix_filename
from gooddata_legacy2cloud.ldm.ads_mapping import ADSMapping
from gooddata_legacy2cloud.ldm.attribute_builder import AttributeBuilder
from gooddata_legacy2cloud.ldm.data_classes import (
    AttributeBuilderConfig,
    FactBuilderConfig,
)
from gooddata_legacy2cloud.ldm.fact_builder import FactBuilder
...
from gooddata_legacy2cloud.ldm.utils import get_cloud_id_date_dimension
...
class CloudModelBuilder:
    ...
    def __init__(
        ...
        self.TagProvider = TagProvider(self.config.legacy_client)
    ...
    def _get_cloud_identifier(
        self, legacy_identifier: str, dataset: str, log_mapping=True
    ):
        """
        Creates a Cloud identifier based on the Legacy model.
        """
        new_id = transform_legacy_identifier(legacy_identifier, dataset)
        if log_mapping:
            self.mappings.add_mapping_identifier(legacy_identifier, new_id)
        return new_id

    def prepare_ws_data_filter(self):
        """
        Prepares a workspace data filter based on the Legacy model.
    ...
    def get_tags(self, legacy_identifier, folder):
        """
        Returns tags for the given identifier.
        """
        tags = self.TagProvider.get_tags(legacy_identifier)
    ...
    def _process_dataset(self, dataset):
        """
        This method is responsible for transforming a Legacy dataset into
        a Cloud dataset. It also logs the transformation process.
        """
        legacy_dataset_id = dataset["identifier"]
        dataset_id = transform_legacy_identifier(dataset["identifier"])
        ...
        logger.write_identifier_relation(legacy_dataset_id, dataset_id)
        ...
        if anchor_attr_id.endswith(".factsof"):
            ...
            if "grain" in anchor_attribute and "labels" not in anchor_attribute:
                # setting up invalid Legacy grain for further postprocessing
        ...
        if "labels" in anchor_attribute:
            ...
            id_pk_column = self.ADSMapping.get_anchor_attribute_column(
                legacy_dataset_id, anchor_attr_id
            )
            ...
            if "sortOrder" in anchor_attribute:
                ...
                new_anchor_attribute["sortColumn"] = (
                    self.ADSMapping.get_anchor_attribute_column(
                        legacy_dataset_id, sort_details["label"]
                    )
                )
        ...
        dataset = {
            ...
            "description": "",
            "facts": facts,
            "grain": grain,  # contains Legacy's format that is adjusted in postprocessing with refereces
            "id": dataset_id,
    ...
    def _process_date_dimensions(self):
        ...
        for date in date_dimensions:
            legacy_id = date["dateDimension"]["identifierPrefix"]
            cloud_id = get_cloud_id_date_dimension(legacy_id)
            ...
            self.mappings.add_mapping_identifier(legacy_id, cloud_id)

            # add additional datetime mappings
            for date_mapping in DATE_MAPPINGS:
                self.mappings.add_mapping_identifier(
                    f"{legacy_id}.{date_mapping[0]}", f"{cloud_id}.{date_mapping[1]}"
                )

    def _process_data_datasets(self):
        """
        Enhances the Cloud model with information from the Legacy model.
        """
        model = self.legacy_model["projectModelView"]["model"]["projectModel"]
    ...
    def process_references_and_grains(self):
        reference_builder = ReferenceBuilder(
            self.legacy_model, self.ADSMapping, self.mappings
        )
    ...
    def load_legacy_model(self, legacy_model):
        """
        Load the Legacy model into the CloudModelBuilder.
        """
        ...
        for legacy_id, cloud_id in self.mappings.get().items():
            mapping_logger.write_identifier_relation(legacy_id, cloud_id)
