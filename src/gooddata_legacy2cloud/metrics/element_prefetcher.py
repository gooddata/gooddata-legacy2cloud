...
"""
Element prefetching utility for batch fetching element values from Legacy.
...
class ElementPrefetcher:
    ...
    def __init__(self, legacy_client: LegacyClient):
        ...
        Args:
            legacy_client: Legacy API instance for making requests
        """
    ...
    def collect_element_uris_from_objects(self, objects: list[dict[str, Any]]) -> None:
        ...
        Args:
            objects: List of Legacy objects (insights, reports, dashboards, etc.)
        """
        for obj in objects:
    ...
    def _fetch_elements_for_attribute(
        ...
        3. Fetches each batch via the Legacy API
        ...
        # Step 1: Get the primary display form ID (required for Legacy API)
        try:
            attribute_obj = self.legacy_client.get_object(attribute_uri)
            display_forms = (
        ...
        for batch_idx in range(num_batches):
            ...
            try:
                result = self.legacy_client.fetch_valid_elements(
    ...
    def _process_valid_elements_response(
        ...
        for item in items:
            ...
            if uri and title is not None:
                ...
                self.legacy_client.cache_attribute_elements[uri] = title
    ...
    def _create_single_metric(
        ...
        try:
            ...
            metric_uri = self._create_legacy_metric(metric_name, maql_expression)
    ...
    def create_metrics_for_unmapped_elements(self) -> None:
        """
        Create Legacy metrics for unmapped elements to enable validation-based lookup.

        For each attribute with unmapped elements, creates metrics with expressions
        that reference those elements. When Legacy validates these metrics, it returns
        ...
        logger.info(
            "----Creating Legacy metrics for unmapped attributes (%s)----",
            num_attributes,
        )
    ...
    def _create_legacy_metric(self, title: str, expression: str) -> str:
        """
        Create a Legacy metric with the given title and MAQL expression.
        ...
        """
        url = f"{self.legacy_client.domain}/gdc/md/{self.legacy_client.pid}/obj"
        payload = {
            "metric": {
                "meta": {
                    "title": title,
        ...
        response = self.legacy_client._post(url, payload)
    ...
    def delete_created_metrics(self) -> None:
        ...
        logger.info(
            "----Deleting created Legacy metrics (%s)----",
        ...
        for idx, metric_uri in enumerate(self.created_metric_uris, 1):
            ...
            try:
                url = f"{self.legacy_client.domain}{metric_uri}"
                self.legacy_client._delete(url)
                logger.info("Done")
            except Exception as e:
                logger.error("ERROR: %s", e)
