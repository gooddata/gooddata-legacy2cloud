...
class DateFilterPostProcessor:
    ...
    - Selection priority: most cyclical filters, then earliest position in original Legacy filters
    """
    ...
    def process(
        ...
        Args:
            ...
            legacy_filters (list): Original Legacy filters (for tracking order)
            attribute_filter_configs (dict): Filter configurations
            cyclical_filter_info (dict): Info about which Legacy filters produced cyclical results
        ...
        dimension_analysis = self._analyze_date_dimensions(
            cloud_filters, legacy_filters, cyclical_filter_info
        )
    ...
    def _analyze_date_dimensions(
        self, cloud_filters, legacy_filters, cyclical_filter_info
    ):
        """
        ...
        Args:
            cloud_filters (list): List of processed Cloud filters
            legacy_filters (list): Original Legacy filters for order tracking
            cyclical_filter_info (dict): Info about which Legacy filters produced cyclical results
        ...
        for legacy_index, info in cyclical_filter_info.items():
    ...
    def _create_friendly_filter_description(self, legacy_filter, dim_id):
        """
        Create a user-friendly description of a Legacy filter for warning messages.

        Args:
            legacy_filter (dict): The original Legacy filter
        ...
        """
        try:
            # Extract attribute URI from Legacy filter
            attr_uri = _get_attribute_uri_from_filter(legacy_filter)
            if not attr_uri:
                return f"{dim_id} - unknown attribute"

            # Get the Legacy attribute identifier
            ...
            element_uris = self._extract_element_uris_from_filter(legacy_filter)

            # Try to convert elements to readable values for cyclical filters
            if element_uris and _is_cyclical_legacy_attribute(attribute_identifier):
    ...
    def _extract_element_uris_from_filter(self, legacy_filter):
        """
        Extract element URIs from a Legacy filter.

        Args:
            legacy_filter (dict): The Legacy filter object

        Returns:
            list: List of element URIs
        """
        element_uris = []
        tree = legacy_filter.get("tree", {})
    ...
    def _select_primary_dimension(self, dimension_analysis):
        ...
        # Sort by: most cyclical filters (desc), then earliest legacy_index (asc) for tie-breaking
        primary_dim = sorted(
            cyclical_dimensions.items(),
            key=lambda x: (-len(x[1]["cyclical_filters"]), x[1]["legacy_index"]),
        )[0][0]
    ...
    def _remove_non_primary_cyclical_filters(
        ...
        Args:
            ...
            dimension_analysis (dict): Analysis of date dimensions
            legacy_filters (list): Original Legacy filters for warning messages
            attribute_filter_configs (dict): Filter configurations
        ...
        for dim_id, info in dimension_analysis.items():
            ...
            for filter_info in info["cyclical_filters"]:
                ...
                # Issue warning for each removed Legacy filter
                legacy_index = filter_info["legacy_index"]
                if legacy_index < len(legacy_filters):
                    legacy_filter = legacy_filters[legacy_index]
                    friendly_description = self._create_friendly_filter_description(
                        legacy_filter, dim_id
                    )
    ...
    def _reorder_primary_cyclical_filters(
        ...
        # Collect primary dimension's cyclical filters with Legacy order
        ...
        for cf_info in primary_info["cyclical_filters"]:
            for i, pf in enumerate(cloud_filters):
                if pf is cf_info["filter"]:
                    primary_cyclical_filters.append((cf_info["legacy_index"], pf))
                    primary_cyclical_indices.add(i)
                    break

        # Sort cyclical filters by original Legacy order
