...
"""
Ranking filter class for Legacy to Cloud filter migration.
...
class RankingFilter(Filter):
    ...
    def process(
        ...
        """
        Process a ranking filter from Legacy to Cloud format.
        ...
        try:
            ...
            measure_obj = self.ctx.legacy_client.get_object(measure_uri)
            legacy_measure_identifier = measure_obj["metric"]["meta"]["identifier"]
            ...
            for uri, local_id in metric_local_ids.items():
                try:
                    # Get the Legacy metric object for comparison
                    displayed_metric_obj = self.ctx.legacy_client.get_object(uri)
                    displayed_legacy_identifier = displayed_metric_obj["metric"][
                        "meta"
                    ]["identifier"]
                    if displayed_legacy_identifier == legacy_measure_identifier:
            ...
            if legacy_measure_identifier in metric_local_ids:
                # Use the provided local ID from the context
                measure_local_id = metric_local_ids[legacy_measure_identifier]
            else:
                # Look up the identifier in the metric mappings
                measure_local_id = self.ctx.metric_mappings.search_mapping_identifier(
                    legacy_measure_identifier
                )

            # Handle ranking filter granularity according to Cloud limitations
            matching_df_id = None

            # Case 1: No attributes in Legacy ranking filter - use empty granularity in Cloud
            if not attribute_uris:
                # No attributes in Legacy, no attributes in Cloud - this is fine
                matching_df_id = None

            # Case 2: Exactly one attribute in Legacy ranking filter
            elif len(attribute_uris) == 1:
                attribute_uri = attribute_uris[0]
                try:
                    # Validate the attribute - especially important for date attributes
                    attr_obj = self.ctx.legacy_client.get_object(attribute_uri)
            ...
            # Case 3: Multiple attributes in Legacy ranking filter - check for exact granularity match
            else:
