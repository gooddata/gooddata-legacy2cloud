        ...
        if not dataset_id or dataset_id not in ctx.ldm_mappings.get().values():
            try:
                ...
                for key, value in ctx.ldm_mappings.get().items():
                    if legacy_dataset_name.lower() in key.lower() and value.startswith(
                        "dt_"
                    ):
                        dataset_id = value
                        break
