...
"""
Chart sorting for Legacy to Cloud migration.
        ...
        for sort_item in chart_sort.get("x", []):
            if "metricChartSort" in sort_item:
                            ...
                            if uri:
                                try:
                                    ...
                                    if "metric" in legacy_obj:
                                        legacy_metric_identifier = legacy_obj["metric"][
                                            "meta"
                                        ]["identifier"]
                                    else:
                ...
                try:
                    ...
                    label_obj = ctx.legacy_client.get_object(attribute_uri)
                    if "attributeDisplayForm" in label_obj:
                        form_uri = label_obj["attributeDisplayForm"]["content"].get(
                            "formOf", ""
                        )
                        if form_uri:
                            attr_obj = ctx.legacy_client.get_object(form_uri)
