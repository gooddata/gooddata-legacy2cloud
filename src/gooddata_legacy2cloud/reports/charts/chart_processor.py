...
"""
Chart processing implementation for Legacy to Cloud migration.
"""
...
def process_chart_report(ctx: ContextWithWarnings, report, content):
    """
    Process a Legacy chart report into a Cloud visualizationObject.
    ...
    if chart.get("type") == "thermometer":
        ...
        from gooddata_legacy2cloud.reports.visualization_adjustments import (
            adjust_buckets,
        )
    ...
    # Build a map of Legacy URIs to local IDs for filter conversion
    # This needs to map Legacy URIs (used in filters) to local IDs (used in buckets)
    metric_local_ids = {}

    # Get the original Legacy metrics from the grid section
    legacy_metrics = content.get("grid", {}).get("metrics", [])

    # Build mapping from Legacy URI to Cloud local identifier
    for legacy_metric in legacy_metrics:
        legacy_uri = legacy_metric.get("uri", "")
        if legacy_uri:
            try:
                # Get the Legacy identifier to map to Cloud identifier
                legacy_obj = ctx.legacy_client.get_object(legacy_uri)
                legacy_identifier = legacy_obj["metric"]["meta"]["identifier"]
                cloud_local_id = ctx.metric_mappings.search_mapping_identifier(
                    legacy_identifier
                )
                if cloud_local_id:
                    metric_local_ids[legacy_uri] = cloud_local_id
            except Exception:
                # Fallback: use the URI directly as local ID
                metric_local_ids[legacy_uri] = legacy_uri
