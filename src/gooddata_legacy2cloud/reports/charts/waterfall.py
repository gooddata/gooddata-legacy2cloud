...
"""
Waterfall chart processing for Legacy to Cloud migration.
    ...
    if fixed_mapping:
        for metric_uri in fixed_mapping:
            ...
            try:
                ...
                legacy_obj = ctx.legacy_client.get_object(metric_uri)
                if "metric" in legacy_obj:
                    legacy_metric_identifier = legacy_obj["metric"]["meta"][
                        "identifier"
                    ]
                else:
                    legacy_metric_identifier = metric_uri
        ...
        for item in bucket_items:
            ...
            if uri and uri not in ["metric", "metricGroup"]:
                ...
                try:
                    ...
                    attr_obj = ctx.legacy_client.get_object(uri)
