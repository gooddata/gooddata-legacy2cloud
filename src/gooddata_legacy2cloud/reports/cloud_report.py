...
which transforms a Legacy report (with "reportDefinition")
into a Cloud visualizationObject.
It uses the transformation logic in transformation.py where each Legacy URI is first resolved to its Legacy identifier
...
from gooddata_legacy2cloud.reports.transformation import transform_legacy_report
from gooddata_legacy2cloud.reports.warning_collector import WarningCollector


class CloudReport:
    """
    The CloudReport class transforms a Legacy report into a Cloud visualizationObject.
        ...
        # Original merged reports format directly uses reportDefinition
            ...
            for key in metadata:
                if (
                    ...
                    self.legacy_report = {"reportDefinition": metadata[key]}
                    break
            else:
    ...
    def get(self):
        ...
        """
        return transform_legacy_report(
            self.ctx, self.legacy_report, self.warning_collector
        )
