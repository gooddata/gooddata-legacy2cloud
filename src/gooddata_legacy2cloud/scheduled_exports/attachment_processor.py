...
from gooddata_legacy2cloud.helpers import validate_non_null_string
...
from gooddata_legacy2cloud.models.legacy.visualization_objects import (
    VisualizationObjectWrapper,
)
from gooddata_legacy2cloud.models.legacy.visualization_widget import (
    VisualizationWidgetWrapper,
)
from gooddata_legacy2cloud.scheduled_exports.helpers.constants import (
    ALL_TIME_DATE_FILTER,
)
from gooddata_legacy2cloud.scheduled_exports.helpers.mappers import CloudMaps, Exports
...
class AttachmentProcessor:
    """Process Legacy scheduled email attachments and convert them to Cloud exports."""
    ...
    def _get_filter_context_translator(self, filter_context_uri) -> FilterContext:
        ...
        filter_context_translator_context = DashboardContext(
            legacy_client=self.context.backends.legacy_client,
    ...
    def _get_legacy_visualization_object(
        self, attachment_uri: str
    ) -> VisualizationObjectWrapper:
        """Get the Legacy visualization object from the attachment URI."""
        raw_legacy_widget_object = self.context.backends.legacy_client.get_object(
            attachment_uri
        )
        legacy_widget_object = VisualizationWidgetWrapper(**raw_legacy_widget_object)
        raw_legacy_visualization_object = (
            self.context.backends.legacy_client.get_object(
                legacy_widget_object.visualization_widget.content.visualization
            )
        )
        legacy_visualization_object = VisualizationObjectWrapper(
            **raw_legacy_visualization_object
        )
        return legacy_visualization_object

    def _get_cloud_insight_id(self, legacy_visualization_id: str) -> CloudInsightId:
        """Get the Cloud insight ID from the Legacy visualization ID."""
    ...
    def _process_visualization_widget_attachment(
        ...
        legacy_visualization_object = self._get_legacy_visualization_object(
            attachment.uri
        )

        cloud_insight_id = self._get_cloud_insight_id(
            legacy_visualization_object.visualization_object.meta.identifier
        )
        ...
        for raw_format in attachment_formats:
            ...
            # If Legacy attachment has a filter context, we need to translate it to
            ...
            request_payload = TabularExportRequestPayload.from_kwargs(
                ...
                file_name=legacy_visualization_object.visualization_object.meta.title,
    ...
    def _process_kpi_dashboard_attachment(
        ...
        # If Legacy attachment has a filter context, we need to translate it to Cloud
        if attachment.filter_context:
            filter_context_translator = self._get_filter_context_translator(
                attachment.filter_context
            )
            ...
            for filter_ in filter_context_object.attributes.content.filters:
                if (
                    ...
                    # date filter artifact which is created by Legacy backend when
        ...
        export = VisualExportRequestPayload(
            request_payload=VisualExportRequestPayloadContent(
                ...
                file_name=legacy_email.content.subject,
    ...
    def process_attachments(
        self, legacy_email: ScheduledMail, cloud_dashboard_id: str
    ) -> Exports:
        ...
        attachments: list[Attachment] = legacy_email.content.attachments
        ...
        for attachment in attachments:
            if attachment.visualization_widget_attachment:
                ...
                visual_attachment = self._process_kpi_dashboard_attachment(
                    cloud_dashboard, legacy_email, attachment.kpi_dashboard_attachment
                )
