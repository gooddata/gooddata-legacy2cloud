...
class PeriodComparisonInsight:
    def __init__(
        ...
        self.ctx = ctx
        self.legacy_measure_definition = legacy_definition
        self.obj = None
    ...
    def get_measure(self):
        if (
            "previousPeriodMeasure"
            in self.legacy_measure_definition["measure"]["definition"]
        ):
            legacy_link = self.legacy_measure_definition["measure"]["definition"][
                "previousPeriodMeasure"
            ]["dateDataSets"][0]["dataSet"]["uri"]
            self.obj = self.ctx.legacy_client.get_object(legacy_link)
            ...
            measure = self._get_measure_previous_period(
                new_dataset_id,
                self.legacy_measure_definition["measure"]["localIdentifier"],
                self.legacy_measure_definition["measure"]["definition"][
                    "previousPeriodMeasure"
                ]["measureIdentifier"],
                self.legacy_measure_definition["measure"]["definition"][
                    "previousPeriodMeasure"
            ...
            # Preserve alias from original Legacy measure if it exists
            if "alias" in self.legacy_measure_definition["measure"]:
                measure["measure"]["alias"] = self.legacy_measure_definition["measure"][
                    "alias"
                ]
            return measure
        elif (
    ...
    def _get_dataset_item(self, dataset_uri: str):
        dataset_item_obj = self.ctx.legacy_client.get_object(dataset_uri)
        return {
            "identifier": {
    ...
    def create_insight_object_from_kpi(self):
        buckets = []
        self.obj = self.legacy_measure_definition
        metric_obj = self.ctx.legacy_client.get_object(
        ...
        if "dateDataSet" in self.obj["kpi"]["content"]:
            dataset_obj = self.ctx.legacy_client.get_object(
    ...
    def get(self):
        if "measure" in self.legacy_measure_definition:
            return self.get_measure()
        elif "kpi" in self.legacy_measure_definition:
