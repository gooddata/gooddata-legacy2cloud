...
function showComparison(rowId, legacyUrl, cloudUrl, title) {
    ...
    let legacyNonEmbeddedUrl = '';
    let cloudNonEmbeddedUrl = '';

    if (row) {
        // Find the legacy and cloud ID links (non-embedded URLs)
        const legacyLink = row.querySelector('td:nth-child(4) a');
        const cloudLink = row.querySelector('td:nth-child(5) a');

        if (legacyLink) {
            legacyNonEmbeddedUrl = legacyLink.href;
        }
    ...
    // Set up the "Open in Legacy" button
    const openlegacyButton = document.getElementById('openlegacyButton');
    if (legacyNonEmbeddedUrl) {
        openlegacyButton.onclick = function() { window.open(legacyNonEmbeddedUrl, '_blank'); };
        openlegacyButton.style.display = 'inline-flex'; // Make sure it's visible
        openlegacyButton.classList.remove('hidden');
    } else {
        openlegacyButton.style.display = 'none';
        openlegacyButton.classList.add('hidden');
    }
    ...
    // Special handling for Legacy frame reloading, to ensure reports are reloaded
    // First, detect if this is a report (reports have 'reportWidget.html' in the URL)
    const isReport = legacyUrl.includes('reportWidget.html');
    ...
    if (isReport) {
        ...
        legacyFrame.src = 'about:blank';
        setTimeout(() => {
            legacyFrame.src = legacyUrl;
        }, 50);
    } else {
...
function closeComparison() {
    ...
    document.getElementById('legacyFrame').src = 'about:blank';
