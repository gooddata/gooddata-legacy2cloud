...
from gooddata_legacy2cloud.web_compare_processing.url_utils import (
    ObjectUrls,
    generate_urls,
)
...
def _process_entry(
    ...
    try:
        legacy_title = entry["legacy_title"]
    except KeyError, TypeError:
        legacy_title = f"Unknown Title {index}"

    # Get definitions with fallbacks to empty dict or string
    try:
        legacy_definition = entry.get("legacy_definition", {})
    except Exception:
        legacy_definition = "{}"  # Empty JSON string as fallback
    ...
    legacy_id, legacy_obj_id, cloud_id = extract_ids_from_definitions(
        legacy_definition, cloud_definition
    )

    # If we don't have a Legacy ID, try to get it from the entry itself
    if not legacy_id and "legacy_id" in entry:
    ...
    try:
        from gooddata_legacy2cloud.web_compare_processing.parsing import (
            determine_status,
        )
    ...
    # Extract Cloud title - fall back to Legacy title if not available
    try:
        cloud_title = extract_cloud_title(cloud_definition)
        if not cloud_title:
            cloud_title = legacy_title
    except Exception:
        cloud_title = legacy_title
