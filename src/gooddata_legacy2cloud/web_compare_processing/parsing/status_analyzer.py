...
def determine_success(legacy_def: Any, cloud_def: Any) -> bool:
    """
    ...
    if isinstance(legacy_def, str) and (
        legacy_def.startswith("ERROR:") or "error" in legacy_def.lower()
    ):
        success = False
    ...
    elif isinstance(cloud_def, dict) and isinstance(legacy_def, dict):
        # Check for error fields in the dictionaries
        if "error" in cloud_def or "error" in legacy_def:
            success = False
    ...
    elif isinstance(cloud_def, dict) != isinstance(legacy_def, dict):
        success = False
