...
from gooddata_platform2cloud.arg_parsing.arg_parser import parse_insight_cli_args
...
from gooddata_platform2cloud.config.configuration_objects import InsightConfig
...
PLATFORM_INSIGHTS_FILE = "platform_insights.json"
...
def migrate_insights(config: InsightConfig):
    ...
    logger.info("----Fetching Platform insights----")
    platform_insights = fetch_objects_with_filters(
        platform_client, "visualizationObject", filter_params, "insights"
    )

    # Filter objects based on mapping files if requested
    if config.object_filter_config.without_mapped_objects:
        platform_insights = filter_objects_by_mapping_files(
            platform_insights,
    ...
    if prefetch_required:
        ...
        prefetcher.collect_element_uris_from_objects(platform_insights)
    ...
    if config.object_migration_config.dump_platform:
        write_content_to_file(
            PLATFORM_INSIGHTS_FILE, json.dumps(platform_insights, indent=4)
        )
        logger.info(
            "Platform insights dumped to '%s'", prefix_filename(PLATFORM_INSIGHTS_FILE)
        )

    logger.info("----Processing Platform insights (%d)----", len(platform_insights))
    ...
    if len(platform_insights) > len(cloud_insights):
        logger.error(
            "----%d (out of %d) insights cannot be migrated----",
            len(platform_insights) - len(cloud_insights),
            len(platform_insights),
        )
