...
from gooddata_platform2cloud.arg_parsing.arg_parser import parse_metric_cli_args
...
from gooddata_platform2cloud.config.configuration_objects import MetricConfig
...
PLATFORM_METRICS_FILE = "platform_metrics.json"
...
def migrate_metrics(config: MetricConfig):
    ...
    platform_metrics = fetch_objects_with_filters(
        platform_client, "metric", filter_params, "metrics"
    )

    # Filter objects based on mapping files if requested
    if config.object_filter_config.without_mapped_objects:
        platform_metrics = filter_objects_by_mapping_files(
            platform_metrics,
    ...
    if config.element_values_prefetch:
        ...
        prefetcher.collect_element_uris_from_objects(platform_metrics)
    ...
    if config.object_migration_config.dump_platform:
        write_content_to_file(
            PLATFORM_METRICS_FILE, json.dumps(platform_metrics, indent=4)
        )
        logger.info(
            "Platform metrics dumped to '%s'", prefix_filename(PLATFORM_METRICS_FILE)
        )

    logger.info("----Processing Platform metrics (%d)----", len(platform_metrics))
    ctx = MetricContext(
    ...
    metrics_builder.process_platform_metrics(platform_metrics)
    ...
    if len(platform_metrics) > len(cloud_metrics):
        logger.error(
            "----%d (out of %d) metrics cannot be migrated----",
            len(platform_metrics) - len(cloud_metrics),
