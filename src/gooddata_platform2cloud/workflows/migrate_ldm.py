...
from gooddata_platform2cloud.arg_parsing.arg_parser import parse_ldm_cli_args
...
from gooddata_platform2cloud.config.configuration_objects import LDMConfig
...
PLATFORM_MODEL_FILE = "platform_ldm.json"
...
def migrate_ldm(config: LDMConfig):
    ...
    platform_ldm = platform_client.get_model()

    if config.object_migration_config.dump_platform:
        write_content_to_file(PLATFORM_MODEL_FILE, json.dumps(platform_ldm, indent=4))
        logger.info(
            "Platform LDM model dumped to '%s'", prefix_filename(PLATFORM_MODEL_FILE)
        )
    ...
    model_builder.load_platform_model(platform_ldm)
