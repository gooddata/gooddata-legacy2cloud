...
from gooddata_platform2cloud.arg_parsing.arg_parser import parse_report_cli_args
...
from gooddata_platform2cloud.backends.platform.objects import fetch_reports_with_filters
from gooddata_platform2cloud.config.configuration_objects import ReportConfig
...
from gooddata_platform2cloud.reports.transformation import set_report_title_prefix

PLATFORM_REPORTS_FILE = "platform_reports.json"
...
def migrate_reports(config: ReportConfig):
    ...
    if config.element_values_prefetch:
        ...
        prefetcher.collect_element_uris_from_objects(platform_reports)
    ...
    if config.object_migration_config.dump_platform:
        write_content_to_file(
            PLATFORM_REPORTS_FILE, json.dumps(platform_reports, indent=4)
        )
        logger.info(
            "Platform reports dumped to '%s'", prefix_filename(PLATFORM_REPORTS_FILE)
        )

    logger.info("----Processing Platform reports (%d)----", len(platform_reports))
    ...
    if len(platform_reports) > len(cloud_reports):
        logger.error(
            "----%d (out of %d) reports cannot be migrated----",
            len(platform_reports) - len(cloud_reports),
            len(platform_reports),
        )
