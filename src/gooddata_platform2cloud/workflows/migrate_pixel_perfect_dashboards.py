...
By default, each Platform Pixel Perfect dashboard is migrated one-to-one into a single
Cloud KPI dashboard that uses native dashboard tabs (one Platform tab -> one Cloud tab).
...
from gooddata_platform2cloud.constants import PIXEL_PERFECT_DASHBOARD_MAPPING_FILE
...
PLATFORM_DASHBOARDS_FILE = "platform_pp_dashboards.json"
CLOUD_DASHBOARDS_FILE = "cloud_pp_dashboards.json"
CLOUD_FAILED_DASHBOARDS_FILE = "platform_failed_pp_dashboards.json"
CLOUD_SKIPPED_DASHBOARDS_FILE = "platform_skipped_pp_dashboards.json"
...
def migrate_pixel_perfect_dashboards(config: PixelPerfectDashboardConfig) -> None:
    ...
    transformation_logger.write_migration_metadata(
        platform_hostname=platform_client.domain,
        platform_ws=platform_client.pid,
    ...
    logger.info("----Fetching Platform pixel perfect dashboards----")
    # Get Platform pixel perfect dashboards
    platform_dashboards = fetch_objects_with_filters(
        platform_client, DASHBOARD_TYPE, filter_params, "pixel perfect dashboards"
    )
    ...
    logger.info(
        "----Processing Pixel Perfect Dashboards (%d)----", len(platform_dashboards)
    )
