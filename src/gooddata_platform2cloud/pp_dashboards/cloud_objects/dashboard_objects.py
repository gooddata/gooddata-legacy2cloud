...
from gooddata_platform2cloud.helpers import PP_DASHBOARD_PREFIX
from gooddata_platform2cloud.pp_dashboards.platform_objects.pixel_perfect_dashboard import (
    PixelPerfectDashboard,
    Tab,
)
from gooddata_platform2cloud.pp_dashboards.utils import get_migration_id
...
class CloudDashboard(BaseModel):
    ...
    @classmethod
    def create_from_platform_definition(
        cls, pixel_perfect_dashboard: PixelPerfectDashboard, tab: Tab, tab_idx: int
    ):
        return cls(
            id=get_migration_id(
                prefix=PP_DASHBOARD_PREFIX,
                platform_identifier=f"{pixel_perfect_dashboard.meta.identifier}_{tab.identifier}",
                platform_title=f"{pixel_perfect_dashboard.meta.title}",
            ),
            attributes=Attributes(
    ...
    @classmethod
    def create_tabbed_from_platform_definition(
        cls, pixel_perfect_dashboard: PixelPerfectDashboard
    ):
        """Create a single tabbed Cloud dashboard from a Platform PP dashboard."""
        return cls(
            id=get_migration_id(
                prefix=PP_DASHBOARD_PREFIX,
                platform_identifier=pixel_perfect_dashboard.meta.identifier,
                platform_title=f"{pixel_perfect_dashboard.meta.title}",
            ),
            attributes=Attributes(
