...
from gooddata_platform2cloud.pp_dashboards.utils import Meta
...
class Elements(BaseModel):
    ...
    def initialize_from_platform(self):
        """
        Fetch and initialize elements from Platform API.
        ...
        if self.ctx and isinstance(self.elements, str):
            try:
                self.elements = AttributeElements.model_validate(
                    self.ctx.platform_client.get_object(self.elements)
                )
...
class AttributeDisplayForm(BaseModel):
    ...
    def initialize_from_platform(self):
        """
        Fetch and initialize formOf object from Platform API.
        This must be called explicitly after setting context.
        """
        ...
        if hasattr(self.links, "ctx"):
            ...
            if hasattr(self.links, "initialize_from_platform"):
                self.links.initialize_from_platform()

        try:
            # Fetch and validate the formOf Attribute
            self.content.formOf = Attribute.model_validate(
                self.ctx.platform_client.get_object(obj_link=self.content.formOf)
            )
            ...
            if hasattr(self.content.formOf, "content") and hasattr(
                ...
                for display_form in self.content.formOf.content.displayForms:
                    if hasattr(display_form, "links") and hasattr(
                        ...
                        if hasattr(display_form.links, "initialize_from_platform"):
                            display_form.links.initialize_from_platform()
        except Exception as e:
...
class FilterItemContent(BaseModel):
    ...
    def initialize_from_platform(self):
        """
        Fetch and initialize filter object data from Platform API.
        This must be called explicitly after setting context.
        """
        ...
        try:
            # Fetch the filter object from Platform. For PP dashboards this is typically an
            # attributeDisplayForm, but Platform can also reference prompts (filtered variables)
            # which are not supported in Cloud dashboards.
            raw_obj: dict[str, Any] = cast(
                dict[str, Any], self.ctx.platform_client.get_object(obj_link=self.obj)
            )

            if "prompt" in raw_obj:
                self.unsupported_reason = (
                    "Platform prompt/filtered variable (not supported in Cloud dashboards): "
            ...
            if hasattr(self.obj, "ctx"):
                ...
                if hasattr(self.obj, "initialize_from_platform"):
                    self.obj.initialize_from_platform()

            # Set default elements (convert Platform element URIs to Cloud values)
            if self.type == "list" and isinstance(
                ...
                for uri in element_uris:
                    try:
                        from gooddata_platform2cloud.metrics.attribute_element import (
                            AttributeElement,
                        )
            ...
            # This avoids relying on Platform display-form identifiers which may not exist in mappings.
