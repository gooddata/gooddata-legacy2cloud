...
from gooddata_platform2cloud.pp_dashboards.platform_objects.filter_objects import (
    FilterItemContent,
)
...
class ReportItem(WrapperUnwrapMixin, Item):
    ...
    def initialize_from_platform(self):
        """
        Fetch and initialize report object from Platform API.
        This must be called explicitly after setting context.
        """
        if self.ctx and isinstance(self.obj, str):
            try:
                obj = self.ctx.platform_client.get_object(obj_link=self.obj)
...
class HeadlineItem(WrapperUnwrapMixin, Item):
    ...
    def initialize_from_platform(self):
        """
        Fetch and initialize metric object from Platform API.
        This must be called explicitly after setting context.
        """
        if self.ctx and isinstance(self.metric, str):
            try:
                obj = self.ctx.platform_client.get_object(obj_link=self.metric)
...
class PixelPerfectDashboard(WrapperUnwrapMixin, BaseModel):
    ...
    def initialize_nested_objects(self):
        """
        Initialize all nested objects by propagating context and fetching data from Platform.
        ...
        for filter_content in self.content.filters:
            if hasattr(filter_content, "ctx"):
                ...
                if hasattr(filter_content, "initialize_from_platform"):
                    filter_content.initialize_from_platform()
        ...
        for tab in self.content.tabs:
            for item in tab.items:
                if hasattr(item, "ctx"):
                    item.ctx = self.ctx
                    if hasattr(item, "initialize_from_platform"):
                        item.initialize_from_platform()  # type: ignore
