...
def prefetch_platform_objects(ctx: PPDashboardContext, obj_values: list[Any]) -> None:
    """Asynchronously prefetch Platform objects to warm the cache.

    Uses ctx.platform_client.get_object() to fetch objects concurrently.
    ...
    Args:
        ctx: Migration context containing Platform API client
        obj_values: List of object URIs or references to prefetch
    """

    def _fetch(v: Any) -> None:
        try:
            ctx.platform_client.get_object(obj_link=v)
