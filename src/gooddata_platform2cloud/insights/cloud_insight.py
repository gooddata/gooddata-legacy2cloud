...
which is responsible for transforming the Platform insight to Cloud format.
"""
...
from gooddata_platform2cloud.metrics.contants import DELETED_VALUE
...
class CloudInsight:
    """
    The CloudInsight class is responsible for transforming the Platform insight to Cloud format.
    """
    ...
    def _transform_identifier_in_boolean_filter(self, filter: dict, filter_name: str):
        obj = self.ctx.platform_client.get_object(
    ...
    def _transform_identifier_in_date_filter(self, filter: dict, filter_name: str):
        obj = self.ctx.platform_client.get_object(filter[filter_name]["dataSet"]["uri"])
    ...
    def _process_cyclical_date_filter(
        ...
        try:
            ...
            # Fetch the Platform display form object to get its identifier
            display_form_obj = self.ctx.platform_client.get_object(display_form_uri)

            # Extract Platform identifier from the display form object
            if "attributeDisplayForm" not in display_form_obj:
                # Not a display form, return None to fallback
                return None, []

            platform_identifier = display_form_obj["attributeDisplayForm"]["meta"][
                "identifier"
            ]

            # Check if this is a cyclical date attribute (using Platform identifier)
            if not _is_cyclical_platform_attribute(platform_identifier):
            ...
            # Convert using cyclical date conversion (using Platform identifier)
            converted_values, missing_elements, null_elements = (
                convert_cyclical_date_elements(
                    self.ctx, element_uris, platform_identifier
                )
            )
    ...
    def _transform_bucket(
        self, mapping: IdMappings, bucket: dict, platform_object: dict, bucket_type: str
    ):
        cloud_id = mapping.search_mapping_identifier(
            platform_object[bucket_type]["meta"]["identifier"]
        )
        bucket["measure"]["definition"]["measureDefinition"]["item"] = {
            "identifier": {
                "id": cloud_id,
                "type": platform_object[bucket_type]["meta"]["category"],
            }
        }
    ...
    def _get_new_measures(self, bucket: dict):
        ...
        for item in bucket["items"]:
            if "arithmeticMeasure" in item["measure"]["definition"]:
                ...
                platform_link = item["measure"]["definition"]["measureDefinition"][
                    "item"
                ]["uri"]
                obj = self.ctx.platform_client.get_object(platform_link)
    ...
    def _get_new_attributes(self, bucket: dict):
        new_items = []
        for item in bucket["items"]:
            platform_link = item["visualizationAttribute"]["displayForm"]["uri"]
            obj = self.ctx.platform_client.get_object(platform_link)
            item["displayForm"] = {
                "identifier": {
                    "id": self.ctx.ldm_mappings.search_mapping_identifier(
    ...
    def _get_visualization_url(self, visualization_class_uri: str):
        obj = self.ctx.platform_client.get_object(visualization_class_uri)
