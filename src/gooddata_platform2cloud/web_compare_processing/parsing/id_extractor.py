...
def extract_ids_from_definitions(
    platform_definition: Any, cloud_definition: Any
) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """
    Extract IDs from Platform and Cloud definitions.
    Handles cases where the definitions might be strings or other non-dictionary types.

    Args:
        platform_definition: Platform object definition
        cloud_definition: Cloud object definition

    Returns:
        Tuple of (platform_id, platform_obj_id, cloud_id)
    """
    try:
        # Extract all IDs using helper functions
        platform_id = extract_platform_id(platform_definition)
        platform_obj_id = extract_platform_obj_id(platform_definition)
        cloud_id = extract_cloud_id(cloud_definition)

        return platform_id, platform_obj_id, cloud_id
    except Exception as e:
        print(f"Warning: Error extracting IDs from definitions: {e}")
        return None, None, None


def extract_platform_id(platform_definition: Any) -> Optional[str]:
    """
    Extract platform_id from Platform definition.

    Args:
        platform_definition: Platform object definition

    Returns:
        Extracted platform_id or None if not found
    """
    if not platform_definition:
        return None

    # Check if we have a dictionary
    if isinstance(platform_definition, dict):
        # Find the root object
        root_object = _get_root_object(platform_definition)
        ...
        if isinstance(root_object, dict):
            for key in ["id", "identifier", "platform_id"]:
    ...
    if isinstance(platform_definition, str):
        return _extract_id_from_string(platform_definition)

    return None


def extract_platform_obj_id(platform_definition: Any) -> Optional[str]:
    """
    Extract platform_obj_id (numeric ID) from Platform definition.

    Args:
        platform_definition: Platform object definition

    Returns:
        Extracted platform_obj_id or None if not found
    """
    if not platform_definition:
        return None
    ...
    if isinstance(platform_definition, str):
        uri_match = re.search(
            r'uri[=:][\s]*["\'](.*?)["\']', platform_definition, re.IGNORECASE
        )
        if uri_match:
