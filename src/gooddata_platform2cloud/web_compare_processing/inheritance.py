...
from gooddata_platform2cloud.web_compare_processing.url_utils import generate_urls
...
class InheritanceManager:
    ...
    def add_inherited_objects(
        ...
        existing_platform_ids = {item.platform_id for item in prefixed_result.items}
        ...
        for unprefixed_item in unprefixed_result.items:
            # Skip if this platform ID already exists in prefixed result
            if unprefixed_item.platform_id in existing_platform_ids:
                continue

            # Generate all URLs in a single call
            urls = generate_urls(
                unprefixed_item.platform_id,
                None,  # platform_obj_id
                unprefixed_item.cloud_id,
                prefixed_result.object_type,
                connection_params["platform_domain"],
                connection_params["platform_ws"],
            ...
            inherited_item = ComparisonItem(
                platform_id=unprefixed_item.platform_id,
                platform_title=unprefixed_item.platform_title,
