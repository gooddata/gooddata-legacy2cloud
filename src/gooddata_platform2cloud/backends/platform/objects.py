...
from gooddata_platform2cloud.backends.platform.dependencies import (
    get_object_dependencies,
    get_uris_from_identifiers,
)
from gooddata_platform2cloud.backends.platform.filters import (
    FilterParameters,
    filter_objects_by_creator_profiles,
    filter_objects_by_locked_flag,
    filter_objects_by_tags,
)
from gooddata_platform2cloud.backends.platform.utils import (
...
def cache_objects(platform_client: PlatformClient, objects: list[Any]) -> int:
    """
    ...
    for obj in objects:
        ...
        if uri:
            obj_url = platform_client.domain + uri
            platform_client.cache[obj_url] = obj
            cached_count += 1
    return cached_count
...
def get_objects_by_ids(
    ...
    for obj_id in obj_ids:
        obj_uri = f"/gdc/md/{platform_client.pid}/obj/{obj_id}"
        obj_url = platform_client.domain + obj_uri

        if obj_url in platform_client.cache:
            cached_obj = platform_client.cache[obj_url]
    ...
    batch_size = PLATFORM_REQUEST_PAGE_SIZE
    for i in range(0, len(objects_to_fetch), batch_size):
        ...
        # Make the API call
        url = f"{platform_client.domain}/gdc/md/{platform_client.pid}/objects/get"
...
def fetch_objects_with_filters(
    platform_client: PlatformClient, category, filter_params=None, message_prefix=None
):
    """
    ...
    if filter_params.identifiers_with_deps:
        ...
        object_uris = get_uris_from_identifiers(
            platform_client, filter_params.identifiers_with_deps
        )
        expanded_uris = get_object_dependencies(platform_client, object_uris, category)
        expanded_ids = [uri.split("/")[-1] for uri in expanded_uris]

        objects = get_objects_by_ids(
            platform_client, expanded_ids, category, message_prefix
        )
    elif filter_params.identifiers:
        # This branch handles --only-identifiers parameter
        object_uris = get_uris_from_identifiers(
            platform_client, filter_params.identifiers
        )
        specific_ids = [uri.split("/")[-1] for uri in object_uris]

        objects = get_objects_by_ids(
            platform_client, specific_ids, category, message_prefix
        )
    elif filter_params.object_ids_with_deps:
        # This branch handles --only-object-ids-with-dependencies parameter
        object_uris = [
            f"/gdc/md/{platform_client.pid}/obj/{obj_id}"
        ...
        objects = get_objects_by_ids(
            platform_client, filter_params.object_ids, category, message_prefix
        )
    else:
        # Default case when no specific object filtering parameters are provided
        # Fetch all objects of the category
        objects = platform_client.get_objects_by_category(category, message_prefix)
    ...
    cache_objects(platform_client, objects)

    return objects


def fetch_reports_with_filters(
    platform_client: PlatformClient, filter_params=None, message_prefix="reports"
):
    """
    Specialized function for fetching Platform reports with filtering options.
    ...
    raw_reports = fetch_objects_with_filters(
        platform_client, "report", filter_params, message_prefix
    )
    ...
    if def_links_mapping:
        ...
        definition_objects = get_objects_by_ids(
            platform_client, definition_ids, "reportDefinition", "report definitions"
        )
...
def fetch_dashboard_content(
    ...
    """
    Fetch all dashboard-related content (widgets and filter contexts) to populate PlatformClient's internal cache.
    ...
    # Batch fetch ALL content at once - this populates platform_client.cache automatically
    if all_content_ids:
        logger.info(
        ...
        objects = get_objects_by_ids(
            platform_client, all_content_ids, None, message_prefix
        )
        cache_objects(platform_client, objects)
    else:
