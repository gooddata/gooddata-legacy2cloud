...
from gooddata_platform2cloud.backends.platform.validate_attribute_elements import (
...
from gooddata_platform2cloud.models.platform.metadata import Metadata
from gooddata_platform2cloud.models.platform.scheduled_exports import ScheduledMail
from gooddata_platform2cloud.models.platform.used_by import Entries, QueryResultWrapper
from gooddata_platform2cloud.models.validators import UrlValidator
...
class PlatformClient:
    """A class used to interact with GD Platform server."""

    def __init__(self, domain, pid, login, password):
        """
        Constructs a PlatformClient object with the given
    ...
    def initialize_attribute_elements_cache(self):
        ...
        """
        logger.info("----Validating Platform Workspace----")
    ...
    def _set_super_secured_token(self):
        ...
        See https://help.gooddata.com/doc/growth/en/expand-your-gooddata-platform/api-reference/#section/Use-Cases/Log-in
        """
    ...
    def get_objects_by_category(
        ...
        """
        Generic method to retrieve definition of all Platform objects of a particular category.
        ...
        if all_objects is None:
            all_objects = []
        offset = page * PLATFORM_REQUEST_PAGE_SIZE
        url = f"{self.domain}/gdc/md/{self.pid}/objects/query?category={category}&limit={PLATFORM_REQUEST_PAGE_SIZE}&offset={offset}&deprecated=1"
        ...
        try:
            ...
            if len(objects) == PLATFORM_REQUEST_PAGE_SIZE:
    ...
    def get_model(self):
        """Retrieves and processes the Platform Logical Data Model (LDM)."""
    ...
    def get_object(self, obj_link: str):
        """Retrieves a Platform object by it's URI."""
        url = self.domain + obj_link
        return self._get(url)

    def get_object_by_identifier(self, identifier):
        """Retrieves a Platform object by its identifier."""
    ...
    def fetch_valid_elements(
        ...
        Example:
            >>> platform_client.fetch_valid_elements("634", [
    ...
    def get_metrics(self):
        """
        Retrieves a list of Platform metrics.
    ...
    @staticmethod
    def validate_platform_scheduled_exports(exports: list[Any]) -> list[ScheduledMail]:
        """Validate Platform scheduled exports against schema."""
        validated_exports = [
    ...
    def get_prompt_project_object(self, prompt_link):
        """
        Retrieves a Platform prompt value.
    ...
    def get_insights(self):
        """
        Retrieves a list of Platform insights.
    ...
    def get_insights_list(self):
        """Retrieves and processes the Platform insights."""
        url = f"{self.domain}/gdc/md/{self.pid}/query/visualizationobjects?showAll=1"
        return self._get(url)

    def get_color_palette(self):
        """Retrieves the Platform color palette."""
    ...
    def get_dashboards(self, dashboard_type="analyticalDashboard"):
        """
        Returns a list of Platform dashboards for the specified type.

        Args:
            dashboard_type (str): Dashboard type (default: 'analyticalDashboard').
            Examples: 'analyticalDashboard' for Platform KPI, 'projectDashboard' for Platform PixelPerfect.
    ...
    def get_dashboard_objects(self) -> list[AnalyticalDashboardWrapper]:
        """
        Retrieves a list of Platform KPI dashboards.
    ...
    def get_workspace_users(self, page: int = 0, all_users: list | None = None) -> list:
        """
        Retrieves a list of all Platform users in the workspace with pagination.
        ...
        offset = page * PLATFORM_REQUEST_PAGE_SIZE
        url = f"{self.domain}/gdc/projects/{self.pid}/users/?offset={offset}&limit={PLATFORM_REQUEST_PAGE_SIZE}"
        ...
        try:
            ...
            if len(users) == PLATFORM_REQUEST_PAGE_SIZE:
                return self.get_workspace_users(page + 1, all_users)

            # Only finish the line when we're at the base recursion level
            if page == 0 or len(users) < PLATFORM_REQUEST_PAGE_SIZE:
