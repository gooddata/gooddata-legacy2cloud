...
class ReferenceBuilder:
    ...
    def __init__(self, platform_model, ADSMapping, mappings):
    ...
    def _get_reference_obj(self, current_dataset, reference_id):
        ...
        for dataset in model["datasets"]:
            if dataset["dataset"]["identifier"] == reference_id:
                cloud_id = transform_platform_identifier(reference_id)
                data_type = "STRING"
    ...
    def _get_reference_and_grain(self, source_dataset, grain_item):
        """
        Set composite key based on Platform model.
        @param source dataset: Platform dataset id
    ...
    def get_references_and_grains(self, dataset_index, cloud_model):
        ...
        (as grain contains Platform format that needs to be adjusted requires) for the dataset.
        """
        references = []
        grains = []
        platform_dataset = self.platform_model["projectModelView"]["model"][
            "projectModel"
        ]["datasets"][dataset_index]["dataset"]
        platform_dataset_id = platform_dataset["identifier"]
        cloud_dataset = cloud_model["ldm"]["datasets"][dataset_index]

        if "references" in platform_dataset:
            for reference_id in platform_dataset["references"]:
                references.append(
                    self._get_reference_obj(
                        platform_dataset["identifier"], reference_id
                    )
                )
            ...
            if len(cloud_dataset["grain"]) == 1:
                ...
                for grain_item in cloud_dataset["grain"]:
                    reference, grain_item = self._get_reference_and_grain(
                        platform_dataset_id, grain_item
                    )
