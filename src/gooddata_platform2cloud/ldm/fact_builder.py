...
from gooddata_platform2cloud.ldm.data_classes import FactBuilderConfig
...
class FactBuilder:
    ...
    def __init__(self, config: FactBuilderConfig):
        self.platform_facts = config.platform_fact
    ...
    def _process_facts(self):
        """
        Process the Platform facts and build the facts.
        """
        for fact in self.platform_facts:
            platform_fact_id = fact["fact"]["identifier"]
            cloud_fact_id = transform_platform_identifier(
                platform_fact_id, self.dataset_id
            )

            fact_column = self.ADSMapping.get_fact_column(
                self.platform_dataset_id, platform_fact_id
            )

            folder = fact["fact"].get("folder", None)

            self.logger.write_identifier_relation(platform_fact_id, cloud_fact_id)
            self.facts.append(
                {
                    ...
                    "tags": self.get_tags(platform_fact_id, folder),
