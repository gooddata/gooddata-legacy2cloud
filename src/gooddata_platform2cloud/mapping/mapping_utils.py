...
def filter_objects_by_mapping_files(
    ...
    # Get Platform IDs from the appropriate mapping files
    platform_ids = set()
    ...
    if filter_mode == "default_only":
        ...
        if os.path.exists(primary_file):
            try:
                ...
                logger.info(
                    "Found %d %s Platform identifiers in the default mapping file",
                    len(platform_ids),
                    object_type,
                )
        ...
        if loaded_files:
            platform_ids = set(id_mappings.get().keys())
            logger.info(
            ...
            logger.info(
                "Found %d %s Platform identifiers in the mapping files",
                len(platform_ids),
                object_type,
            )
        else:
    ...
    for obj in objects:
        ...
        if identifier is not None and identifier not in platform_ids:
