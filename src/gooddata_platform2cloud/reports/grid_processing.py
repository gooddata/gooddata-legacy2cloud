...
"""
Functions for processing grid (table) reports in the Platform to Cloud migration.
These functions handle the conversion of Platform grid/table configurations, including
...
from gooddata_platform2cloud.reports.mappings import (
    map_attribute,
    map_metric,
    map_total_entry,
)
...
def process_metrics(ctx: ContextWithWarnings, platform_metrics):
    """
    Process metrics for grid reports.

    Args:
        ctx (ReportContext): The context object containing mappings
        platform_metrics (list): The list of Platform metrics
    ...
    # Map each Platform metric with lookup using ctx.metric_mappings
    for idx, metric in enumerate(platform_metrics, start=1):
        mapped = map_metric(ctx, metric, idx)
        platform_metric_uri = metric.get("uri", "")
        local_id = mapped["measure"]["localIdentifier"]
        metric_local_ids[platform_metric_uri] = local_id
...
def process_row_attributes(
    ctx: ContextWithWarnings, platform_row_attrs, metric_local_ids, platform_metrics
):
    """
    ...
    Args:
        ctx (ReportContext): The context object containing mappings
        platform_row_attrs (list): The list of Platform row attributes
    ...
    for idx, row in enumerate(platform_row_attrs, start=1):
        attr_obj = row.get("attribute", row)
        mapped = map_attribute(ctx, attr_obj, idx)
        platform_attr_uri = attr_obj.get("uri", "")
        local_id = mapped["attribute"]["localIdentifier"]
        row_attr_local_ids[platform_attr_uri] = local_id
        ...
        if totals:
            # In Platform, the totals structure for row attributes is:
            # [
            ...
            for metric_idx, metric in enumerate(platform_metrics):
...
def process_column_attributes(
    ctx: ContextWithWarnings, platform_columns, platform_metrics, metric_local_ids
):
    """
    ...
    Args:
        ctx (ReportContext): The context object containing mappings
        platform_columns (list): The list of Platform column attributes
        platform_metrics (list): The list of Platform metrics
        metric_local_ids (dict): Mapping of metric URIs to local IDs
...
def process_column_widths(
    ...
    Args:
        ctx (ReportContext): The context object containing mappings
        grid (dict): The Platform grid configuration
    ...
    if report_format not in ["line", "chart"] and grid_column_widths:
        ...
        def resolve_attribute_local_id(locator_uri):
            ...
            try:
                obj = ctx.platform_client.get_object(locator_uri)
...
def process_grid_report(ctx: ContextWithWarnings, report, content, report_format):
    """
    Process a Platform grid report into a Cloud visualizationObject.
    ...
    raw_platform_row_attrs = grid.get("rows", [])

    # Separate out rows that equal the string "metricGroup"
    platform_row_attrs = []
    for row in raw_platform_row_attrs:
        if isinstance(row, str) and row == "metricGroup":
            properties["controls"]["measureGroupDimension"] = "rows"
        else:
            platform_row_attrs.append(row)

    platform_metrics = grid.get("metrics", [])

    # Process metrics
    measures, metric_local_ids = process_metrics(ctx, platform_metrics)

    # Process row attributes
    attribute_bucket, row_attr_local_ids = process_row_attributes(
        ctx, platform_row_attrs, metric_local_ids, platform_metrics
    )

    # Process column attributes
    platform_columns = grid.get("columns", [])
    column_bucket, column_attributes = process_column_attributes(
        ctx, platform_columns, platform_metrics, metric_local_ids
    )
