...
class ReportMappings(IdMappings):
    """
    Class for handling Platform to Cloud mappings for reports.
    ...
    def map_metric(
        ...
        """
        Maps a Platform metric to a Cloud measure object.

        Args:
            ctx: The context object with API and mappings
            metric_obj (dict): The Platform metric object
        ...
        # Retrieve the custom alias and format as provided in the Platform report
        custom_alias = metric_obj.get("alias", "")
        custom_format = metric_obj.get("format", "")
        try:
            obj = ctx.platform_client.get_object(metric_uri)
            # Expect the object to have the metric definition under the "metric" key
            platform_identifier = obj["metric"]["meta"]["identifier"]
            local_id = ctx.metric_mappings.search_mapping_identifier(
                platform_identifier
            )
            # Use the Platform metric meta title as base title regardless of a custom alias
    ...
    def map_attribute(
        ...
        """
        Maps a Platform attribute to a Cloud attribute object.

        Args:
            ctx: The context object with API and mappings
            attr_obj (dict): The Platform attribute object
        ...
        try:
            ...
            if "attribute" in obj:
                ...
                platform_identifier = obj["attributeDisplayForm"]["meta"]["identifier"]
            else:
                platform_identifier = attr_uri
            local_id = ctx.ldm_mappings.search_mapping_identifier(platform_identifier)
        except Exception:
            local_id = attr_uri
        return {
            "attribute": {
    ...
    def map_chart_metric(
        ...
        """
        Maps a Platform chart metric to a Cloud measure object.

        Args:
            ctx: The context object with API and mappings
            item (dict): The Platform chart metric item
            index (int): The index of the metric for generating fallback names
    ...
    def map_chart_attribute(
        ...
        """
        Maps a Platform chart attribute to a Cloud attribute object.

        Args:
            ctx: The context object with API and mappings
            item (dict): The Platform chart attribute item
            index (int): The index of the attribute for generating fallback names
