...
It processes Platform reports in parallel and produces a merged report object for migration.
"""
...
from gooddata_platform2cloud.reports.cloud_report import CloudReport
from gooddata_platform2cloud.reports.data_classes import ReportContext
from gooddata_platform2cloud.reports.payload_validator import (
...
class CloudReportsBuilder:
    ...
    Processes Platform reports in parallel where each report manages its own warning collection
    and thread-safe logging ensures clean log files during migration.
    """

    def __init__(self, ctx: ReportContext):
        self.platform_reports_raw = None
        self.ctx = ctx
        self.cloud_reports = []
    ...
    def process_platform_reports(self, platform_reports_raw: list):
        """
        Load Platform reports. For each report, identify the last report definition,
        ...
        """
        self.platform_reports_raw = platform_reports_raw
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            results = list(
                executor.map(
                    lambda pair: worker(*pair), enumerate(self.platform_reports_raw)
                )
            )
