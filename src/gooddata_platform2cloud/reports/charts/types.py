...
"""
Chart type detection and visualization URL mapping for Platform to Cloud migration.

This module handles determining the type of chart (line, bar, etc.) and its
properties (orientation, stacking) based on the Platform chart configuration.
"""
...
def is_horizontal_bar_chart(chart):
    """
    Determine if a Platform bar chart is horizontal (maps to Cloud 'bar') or
    ...
    """
    # In Platform, if x contains attributes and y contains metrics, it's a vertical bar chart (Cloud column)
...
def is_stacked_chart(chart_type):
    """
    Determine if a Platform chart type is stacked.

    Args:
        chart_type (str): The Platform chart type
...
def process_chart_type_and_visualization(chart, content=None):
    """
    Determines the chart type and visualization URL based on the Platform chart configuration.
