...
"""
Date filter class for Platform to Cloud filter migration.
...
from gooddata_platform2cloud.reports.common import contains_node_type
...
from gooddata_platform2cloud.reports.filters.date_cyclical_processor import (
    CyclicalDateProcessor,
)
from gooddata_platform2cloud.reports.filters.date_helpers import (
    get_date_dataset_and_granularity,
)
from gooddata_platform2cloud.reports.filters.date_ordinal_processor import (
...
class DateFilter(Filter):
    ...
    def process_between_date(self, filter_obj):
        """
        Process a between date filter (relative date filter) from Platform to Cloud format.
        ...
        if _is_cyclical_platform_attribute(attribute_identifier):
    ...
    def process_date_null(self, attr_uri: str, is_negative: bool = False):
        """
        Process a date NULL / NOT NULL filter from Platform to Cloud format.
    ...
    def process_equality_date(self, filter_obj):
        """
        Process a date equality filter (attribute = time macro) from Platform to Cloud format.
    ...
    def process(self, filter_obj, **kwargs):
        """
        Process a date filter from Platform to Cloud format.
