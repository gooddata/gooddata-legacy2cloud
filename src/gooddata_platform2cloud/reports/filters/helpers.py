...
def find_visible_display_form(ctx, attribute_uri, displayed_attributes):
    ...
    """
    try:
        attr_obj = ctx.platform_client.get_object(attribute_uri)
        if (
...
def get_attribute_uri_from_display_form(ctx: ContextWithWarnings, display_form_uri):
    ...
    """
    try:
        obj = ctx.platform_client.get_object(display_form_uri)
        if (
...
def _find_displayed_form_for_attribute(
    ...
    Returns:
        tuple: (display_form_obj, identifier) where display_form_obj is the found
               display form object and identifier is its Platform identifier, or (None, None) if not found
    """
    try:
        ...
        for displayed_uri in displayed_attribute_uris:
            ...
            if parent_attr_uri == attribute_uri:
                ...
                try:
                    df_obj = ctx.platform_client.get_object(displayed_uri)
...
def check_exact_granularity_match(
    ...
    try:
        ...
        for attr_uri in parent_attributes:
            try:
                attr_obj = ctx.platform_client.get_object(attr_uri)
                attr_identifier = (
                    attr_obj.get("attribute", {}).get("meta", {}).get("identifier", "")
                )
...
def get_metric_identifier_for_warnings(ctx: ContextWithWarnings, measure_uri):
    ...
    Returns:
        str: The metric identifier (Cloud if available, otherwise Platform identifier or URI)
    """
    try:
        # Get the Platform identifier
        metric_obj = ctx.platform_client.get_object(measure_uri)
        if (
            metric_obj
            ...
            if cloud_identifier:
                ...
                # Fallback to Platform identifier if mapping not found
                return platform_identifier
        else:
            # If couldn't get Platform ID, use just the URI
            return measure_uri
    except Exception:
