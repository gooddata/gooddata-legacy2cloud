...
"""
Measure filter class for Platform to Cloud filter migration.

This module provides a class for handling measure value filters.
"""

from gooddata_platform2cloud.models.cloud.identifier import (
    Identifier,
    IdentifierWrapper,
)
from gooddata_platform2cloud.models.cloud.insight_filters import (
    Comparison,
    ComparisonWrapper,
    MeasureFilterModel,
...
class MeasureFilter(Filter):
    ...
    def _map_platform_attribute_uri_to_cloud_id(self, attribute_uri: str) -> str | None:
        """Map a Platform attribute URI to a Cloud identifier."""
        try:
            obj = self.ctx.platform_client.get_object(attribute_uri)
            platform_identifier: str | None = None

            if "attribute" in obj and "meta" in obj["attribute"]:
                platform_identifier = obj["attribute"]["meta"].get("identifier")
            elif (
                "attributeDisplayForm" in obj and "meta" in obj["attributeDisplayForm"]
            ):
                platform_identifier = obj["attributeDisplayForm"]["meta"].get(
                    "identifier"
                )

            if not platform_identifier:
                platform_identifier = attribute_uri

            mapped = self.ctx.ldm_mappings.search_mapping_identifier(
                platform_identifier
            )
            return mapped if mapped else platform_identifier
        except Exception:
    ...
    def get_measure_cloud_id(self, measure_uri, metric_cloud_ids):
        ...
        Returns:
            tuple: (measure_local_id, platform_identifier) or (measure_uri, None) if an error occurs
        """
        try:
            # Get the metric object to extract the identifier
            metric_obj = self.ctx.platform_client.get_object(measure_uri)
            platform_identifier = metric_obj["metric"]["meta"]["identifier"]

            # Get the local ID for the metric
            if platform_identifier in metric_cloud_ids:
                # Use the provided local ID from the context
                measure_cloud_id = metric_cloud_ids[platform_identifier]
            else:
    ...
    def process(self, filter_obj, metric_local_ids=None, **kwargs):
        """
        Process a measure value filter from Platform to Cloud format.
        ...
        if granularity_attrs:
            ...
            mapped_ids = [
                self._map_platform_attribute_uri_to_cloud_id(uri)
