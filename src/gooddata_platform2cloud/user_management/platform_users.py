...
def extract_creator_modifier_ids(objects: list[dict]) -> set[str]:
    """
    Extracts unique creator and modifier user IDs from Platform objects.

    Args:
        objects: List of Platform objects (dashboards, insights, etc.)

    Returns:
        set: A set of unique Platform user profile IDs (without /gdc/account/profile/ prefix)
    """
...
def build_platform_user_email_map(
    users: list[dict], identifier_type: str = "login"
) -> dict[str, str]:
    """
    Builds a mapping from Platform user profile IDs to email addresses or login names.

    Args:
        users: List of Platform user dictionaries
