...
from gooddata_platform2cloud.logging.context import ObjectContext
...
class CloudDashboardsBuilder:
    ...
    def __init__(self, ctx: DashboardContext):
        """Constructor."""
        self.platform_dashboards_raw = None
        self.ctx = ctx
        self.cloud_dashboards = []
        self.public_dashboard_ids = []

    def process_platform_dashboards(
        self, platform_dashboards_raw: list, skip_deploy: bool, overwrite_existing: bool
    ):
        """
        Load Platform dashboards and process them to cloud ones.
        """
        self.platform_dashboards_raw = platform_dashboards_raw
        ...
        self.dashboards_logger.write_migration_metadata(
            self.ctx.platform_client.domain,
            self.ctx.platform_client.pid,
            self.ctx.cloud_client.domain,
            self.ctx.cloud_client.ws,
            self.ctx.client_prefix if hasattr(self.ctx, "client_prefix") else None,
        )

        # Build current batch dashboard mappings (Platform ID -> Cloud ID) for drill conversion
        current_batch_mappings = {}
        for dashboard in self.platform_dashboards_raw:
            try:
                platform_id = dashboard[self.ctx.dashboard_type]["meta"]["identifier"]
                dashboard_title = dashboard[self.ctx.dashboard_type]["meta"]["title"]
                cloud_id = get_cloud_id(dashboard_title, platform_id)
                current_batch_mappings[platform_id] = cloud_id
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [
                ...
                for index, dashboard in enumerate(self.platform_dashboards_raw)
            ]
