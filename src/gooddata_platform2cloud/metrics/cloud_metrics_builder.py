...
from gooddata_platform2cloud.metrics.metrics_sorter import MetricsSorter
from gooddata_platform2cloud.metrics.cloud_metric import CloudMetric
...
class CloudMetricsBuilder:
    ...
    def __init__(self, ctx: MetricContext):
        self.platform_metrics_raw = None
        self.ctx = ctx
        self.cloud_metrics = []

    def process_platform_metrics(self, platform_metrics_raw: list):
        """
        Load Platform metrics and process them to Cloud ones.
        """
        self.platform_metrics_raw = platform_metrics_raw
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [
                ...
                for index, metric in enumerate(self.platform_metrics_raw)
            ]
