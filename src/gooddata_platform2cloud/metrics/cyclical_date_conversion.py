...
"""
Shared cyclical date conversion logic for Platform to Cloud migration.
...
def _get_platform_attribute_type(attribute_identifier):
    """
    Extract the Platform attribute type from the full identifier.
...
def _is_cyclical_platform_attribute(attribute_identifier):
    """
    Check if a Platform date attribute is cyclical based on predefined Platform attribute patterns.

    Args:
        attribute_identifier: The Platform attribute identifier (e.g., "order.day.in.week")
...
def convert_cyclical_date_elements(
    ...
    This function relies on ELEMENT IDs from Platform URIs (e.g., elements?id=2) rather than reading
    the actual element values from the date dimension. This is intentional because:
    1. For STANDARD Platform date dimensions, element IDs follow predictable patterns:
    ...
    # Hardcoded conversion mappings based on Platform element IDs (NOT element values)
    # These mappings assume standard Platform date dimension element ID ordering patterns
    ...
    conversion_rules: dict = {
        ...
        "day.in.week": {
            "null_id": 0,
            # Platform element IDs: 1=Sunday, 2=Monday, 3=Tuesday, 4=Wednesday, 5=Thursday, 6=Friday, 7=Saturday
        ...
        "day.in.euweek": {
            "null_id": 0,
            # Platform element IDs: 1=Monday, 2=Tuesday, 3=Wednesday, 4=Thursday, 5=Friday, 6=Saturday, 7=Sunday
    ...
    for uri in element_uris:
        try:
            ...
            # This assumes standard Platform date dimension element ID ordering (e.g., 1=Sunday, 2=Monday, etc.)
