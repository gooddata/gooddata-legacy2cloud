...
"""
Tool to get dependencies of objects on a Legacy domain.
...
from gooddata_legacy2cloud.models.legacy.metrics import MetricWrapper
from gooddata_legacy2cloud.models.legacy.used_by import Entry, QueryResultWrapper

logger = configure_logger("legacy_metric_dependencies")
...
def parse_arguments():
    ...
    workspace_group.add_argument(
        ...
        help="Dynamically fetch all workspaces from the Legacy domain",
        action="store_true",
    )
...
class RequiredEnvVars:
    ...
    def __init__(self, legacy_domain: str, legacy_login: str, legacy_password: str):
    ...
    def from_env(cls, env_file: str | None = None):
        ...
        missing_vars = []
        legacy_login = os.getenv("LEGACY_LOGIN")
        legacy_password = os.getenv("LEGACY_PASSWORD")
        legacy_domain = os.getenv("LEGACY_DOMAIN")

        if legacy_login is None:
            missing_vars.append("LEGACY_LOGIN")
        if legacy_password is None:
            missing_vars.append("LEGACY_PASSWORD")
        if legacy_domain is None:
            missing_vars.append("LEGACY_DOMAIN")
        if missing_vars:
            raise ValueError(
                f"Missing required environment variables: {', '.join(missing_vars)}"
            )
        return cls(
            legacy_domain=os.environ["LEGACY_DOMAIN"],
            legacy_login=os.environ["LEGACY_LOGIN"],
...
def main() -> None:
    ...
    legacy_client = LegacyClient(
        domain=env_vars.legacy_domain,
        pid="placeholder_pid",
        login=env_vars.legacy_login,
        password=env_vars.legacy_password,
    )
    ...
    if args.dynamic_workspace_lookup:
        logger.info("Using dynamic workspace lookup from Legacy domain")
        metadata = legacy_client.get_domain_metadata()
    ...
    for workspace_id in workspace_ids:
        ...
        if object_type == ObjectType.METRIC:
            ...
            raw_objects = legacy_client.get_attributes()
            ...
            query_result = legacy_client.get_insights_query()
        ...
        for input_item in input_items:
            ...
            dependencies = legacy_client.get_object_dependencies(object_uri)
